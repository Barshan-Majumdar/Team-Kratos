...
function checkFrameBrightness(imageSource) {
  try {
    ...
    return avgBrightness >= 35 && avgBrightness <= 225;
  } catch (e) {
    return true;
  }
}
...
export function useLiveness() {
  ...
  const validateAndExtractPose = async (imageSource, requestedPose) => {
    ...
    if (faceapi.nets?.tinyFaceDetector?.isLoaded && faceapi.nets?.faceLandmark68TinyNet?.isLoaded) {
      try {
        detections = await faceapi
          .detectAllFaces(imageSource, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.15 }))
          .withFaceLandmarks(true);
      } catch (err) {
        console.warn('[FaceAPI Detect Error]:', err);
      }
    }

    if (!detections || detections.length === 0) {
      throw new Error('No face detected. Please make sure no objects are blocking your face and position your face inside the circle.');
    }
    ...
    // Object / Face Obstruction check
    const score = detection.detection.score || 0;
    if (score < 0.50) {
      throw new Error('Object detected or face partially obscured. Please remove any objects or hands covering your face.');
    }
    ...
    const isClipped = box.x < 5 || box.y < 5 || (box.x + box.width) > (frameWidth - 5) || (box.y + box.height) > (frameHeight - 5);

    if (areaRatio < 0.12 || offCenterX > 0.30 || offCenterY > 0.30 || isClipped) {
    ...
    if (requestedPose) {
      ...
      switch (poseKey) {
        case 'straight':
          poseMatched = yawRatio > 0.40 && yawRatio < 0.60 && pitchRatio >= 0.38 && pitchRatio <= 0.54;
          break;
        case 'turn_left':
          poseMatched = yawRatio <= 0.37;
          break;
        case 'turn_right':
          poseMatched = yawRatio >= 0.63;
          break;
        case 'tilt_up':
        case 'tilt':
          poseMatched = pitchRatio < 0.37 || pitchRatio > 0.55;
          break;
        default:
          poseMatched = true;
          break;
    ...
    const p = new Promise((resolve, reject) => {
      const timeoutTimer = setTimeout(() => {
        if (verificationPromiseRef.current) {
          ...
          reject(new Error('No face detected in frame. Please position your face inside the circle in good lighting and try again.'));
        }
      }, 15000);
        ...
        if (faceapi.nets?.tinyFaceDetector?.isLoaded && faceapi.nets?.faceLandmark68TinyNet?.isLoaded) {
          try {
            detection = await faceapi
              .detectSingleFace(imageSource, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.15 }))
              .withFaceLandmarks(true);
            if (detection) break;
          } catch (err) {
            console.warn('[FaceAPI Single Frame Notice]:', err);
          }
        }
        if (attempt < 2) {
