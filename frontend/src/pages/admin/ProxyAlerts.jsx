...
const ProxyAlerts = ({ user }) => {
  ...
  const fetchData = async () => {
    ...
    // Fetch alerts and stats independently so one failure doesn't block the other
    try {
      const resAlerts = await fetch(`${API_BASE}/api/proxy-alerts?${params.toString()}`, {
      ...
      if (resAlerts.ok) {
        ...
        console.warn('Proxy alerts fetch returned:', resAlerts.status);
        setAlerts([]);
      }
    } catch (err) {
      console.warn('Failed to fetch proxy alerts:', err.message);
      setAlerts([]);
    }

    try {
      const resStats = await fetch(`${API_BASE}/api/proxy-alerts/stats`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (resStats.ok) {
      ...
      console.warn('Failed to fetch alert stats:', err.message);
    }
              ...
              {loading ? (
                <tr><td colSpan="7" className="p-12 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-sm text-slate-500 font-medium">Loading alerts...</span>
                  </div>
                </td></tr>
              ) : alerts.length === 0 ? (
                <tr><td colSpan="7" className="p-12 text-center">
